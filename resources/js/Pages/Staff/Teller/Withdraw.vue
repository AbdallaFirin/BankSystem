...
const submit             = () => form.post(route('staff.withdraw'), { preserveScroll: false });
...
const applyFilters = () => router.get(route('staff.teller.withdraw'), { date_from: dateFrom.value, date_to: dateTo.value }, { preserveState: true, replace: true });
...
const buildHtml = (rows, title, amtLabel, amtValue, amtColor, extra = '') => `
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .big{font-size:20px;font-weight:700;color:${amtColor}}
...
<div class="body">
  ...
  <div class="row"><span class="lbl" style="font-size:13px;font-weight:600">${amtLabel}</span><span class="big">${amtValue}</span></div>
  ${extra}
</div>
...
const doPrint = (txn) => {
    const extra = txn.is_reversed ? '<div style="text-align:center;margin-top:12px"><span style="display:inline-block;padding:3px 12px;border-radius:20px;font-size:10px;font-weight:700;background:#fde8e8;color:#c0392b;border:1px solid #f5c6c6">REVERSED</span></div>' : '';
    openPrintWindow(buildHtml([
        ['Reference',    txn.reference,      true],
        ['Type',         'Cash Withdrawal',  false],
    ...
    ], 'Cash Withdrawal Receipt', 'Amount Withdrawn', '-$'+fmt(txn.amount), '#b8860b', extra));
};
...
<template>
  <Head title="Cash Withdrawal" />
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-lg bg-[rgba(232,168,48,0.12)] flex items-center justify-center text-[#E8A830] text-lg">
            <i class="ti ti-arrow-up-circle"></i>
          </div>
          <div>
            <h1 class="font-serif text-xl text-[#F0EBE1]">Cash <em class="text-[#E8A830] not-italic">Withdrawal</em></h1>
            <p class="text-[11px] text-[#A9B8C6] mt-0.5">Debit a customer account with cash payout</p>
          </div>
        </div>
        <div class="text-right">
          <p class="text-[10px] text-[#6B7E8E] uppercase tracking-widest">Today's Withdrawals</p>
          <p class="font-serif text-xl text-[#E8A830]">${{ fmt(today_total) }}</p>
        </div>
      </div>
    </template>
    ...
    <div class="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.08)] rounded-2xl overflow-hidden">
          <div class="h-1 bg-[#E8A830]"></div>
          <div class="p-6">
            <h2 class="font-serif text-lg text-[#F0EBE1] mb-6">New Withdrawal</h2>
            <form @submit.prevent="submit" class="space-y-5">
              ...
              <div class="space-y-1.5">
                ...
                <div class="relative">
                  <input type="text" v-model="form.account_number" required placeholder="e.g. GAR002000001"
                         class="w-full font-mono text-sm text-[#F0EBE1] bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] rounded-lg px-4 py-2.5 outline-none focus:border-[#E8A830] transition uppercase"
                ...
                <div v-if="lookup?.found" class="mt-2 p-3 rounded-lg bg-[rgba(232,168,48,0.06)] border border-[#E8A830]/20 text-sm space-y-1">
                  ...
                  <div class="flex justify-between"><span class="text-[#6B7E8E] text-[11px]">Available Balance</span><span class="text-[#E8A830] font-mono font-bold">${{ fmt(lookup.balance) }}</span></div>
              ...
              <div class="space-y-1.5">
                ...
                <input type="number" step="0.01" min="0.01" v-model="form.amount" required placeholder="0.00"
                       ...
                       :class="overLimit
                           ...
                           : 'border-[rgba(255,255,255,0.1)] focus:border-[#E8A830]'" />
                ...
                <div v-if="overLimit"
                  ...
                  <span>
                    ...
                    This withdrawal will be sent to your supervisor for approval before cash is disbursed.
                  </span>
                </div>
              ...
              <div class="space-y-1.5">
                ...
                <textarea v-model="form.remarks" rows="2" placeholder="Additional notes or purpose of this withdrawal…"
                          class="w-full text-sm text-[#F0EBE1] bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] rounded-lg px-4 py-2.5 outline-none focus:border-[#E8A830] transition resize-none placeholder-[#3a5060]"></textarea>
              ...
              <button type="submit" :disabled="form.processing || (lookup !== null && !lookup?.found)"
                      class="w-full disabled:opacity-50 disabled:cursor-not-allowed py-3 rounded-xl font-semibold text-sm transition flex items-center justify-center gap-2"
                      :class="overLimit
                          ? 'bg-amber-500 hover:bg-amber-400 text-white'
                          : 'bg-[#E8A830] hover:bg-[#c9902a] text-[#0B1929]'">
                <i :class="overLimit ? 'ti ti-send text-lg' : 'ti ti-arrow-up-circle text-lg'"></i>
                {{ form.processing ? 'Processing…' : overLimit ? 'Submit for Approval' : 'Process Withdrawal' }}
              </button>
            </form>
          </div>
        </div>
        ...
        <div class="lg:col-span-3 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.08)] rounded-2xl overflow-hidden">
          ...
          <div class="p-5">
            <div class="flex items-center justify-between mb-4">
              <h2 class="font-serif text-lg text-[#F0EBE1]">My Withdrawal History</h2>
              <span class="text-[11px] text-[#6B7E8E]">{{ history.total ?? 0 }} transactions</span>
            </div>

            <div class="flex gap-2 mb-4">
              <input type="date" v-model="dateFrom" class="flex-1 text-xs text-[#F0EBE1] bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-lg px-3 py-2 outline-none focus:border-[#E8A830]" />
              <input type="date" v-model="dateTo"   class="flex-1 text-xs text-[#F0EBE1] bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-lg px-3 py-2 outline-none focus:border-[#E8A830]" />
              <button @click="applyFilters" class="px-3 py-2 rounded-lg bg-[#E8A830]/10 text-[#E8A830] border border-[#E8A830]/20 text-xs hover:bg-[#E8A830]/20 transition">Filter</button>
                ...
                <tbody class="divide-y divide-[rgba(255,255,255,0.04)]">
                  <tr v-if="!history.data?.length">
                    <td colspan="6" class="px-4 py-10 text-center text-[#6B7E8E] text-xs">
                      <i class="ti ti-receipt-off text-2xl block mb-2 opacity-40"></i>No withdrawals yet
                    </td>
                  </tr>
                  <tr v-for="txn in history.data" :key="txn.id"
                    ...
                    <td class="px-3 py-3 text-right font-mono font-bold text-sm"
                        :class="{
                          ...
                          'text-[#E8A830]'             : txn.status === 'completed' && !txn.is_reversed,
                        }">
                      -${{ fmt(txn.amount) }}
                    </td>
            ...
            <div v-if="history.last_page > 1" class="flex justify-end gap-1 mt-4">
              <Link v-for="link in history.links" :key="link.label"
                    ...
                    :class="link.active ? 'bg-[#E8A830] text-[#0B1929] border-[#E8A830] font-bold'
        ...
        <div v-if="reversing" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="cancelReverse">
          <div class="bg-[#112236] border border-[rgba(226,99,90,0.3)] rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="font-semibold text-[#F0EBE1]">Reverse Withdrawal?</h3>
            ...
            <p class="text-[12px] text-[#A9B8C6] mb-5">
              Reversing this withdrawal will <strong class="text-[#E2635A]">deposit ${{ fmt(reversing.amount) }} back</strong> into the customer's account and deduct it from the vault.
            </p>
            <div class="flex gap-3">
